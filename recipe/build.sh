...
if [ "$(uname)" == "Darwin" ]; then
    ...
    ../gcc/configure \
        ...
        --enable-gold \
        --disable-nls \
        --with-build-sysroot=${BUILD_PREFIX}/${HOST}/sysroot \
    ...
    sed -i -e "s@:crt1.o@:${PREFIX}/${HOST}/sysroot/usr/lib/crt1.o@g" ${SPECSFILE}
    sed -i -e "s@ crti.o@ ${PREFIX}/${HOST}/sysroot/usr/lib/crti.o@g" ${SPECSFILE}
    sed -i -e "s@ crtn.o@ ${PREFIX}/${HOST}/sysroot/usr/lib/crtn.o@g" ${SPECSFILE}
    sed -i -e "s@--sysroot=%R@--sysroot=${PREFIX}/${HOST}/sysroot@g" ${SPECSFILE}
    popd
